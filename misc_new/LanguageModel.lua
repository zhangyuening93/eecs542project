...
local LSTM = require 'misc_new.LSTM'
...
function layer:__init(opt)
  ...
  self.extra_input_size = utils.getopt(opt, 'extra_input_size', 512)
  self.num_layers = utils.getopt(opt, 'num_layers', 1)
  local dropout = utils.getopt(opt, 'dropout', 0)
  -- options for Language Model
  self.seq_length = utils.getopt(opt, 'seq_length')
  -- create the core lstm network. note +1 for both the START and END tokens
  self.core = LSTM.lstm(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.extra_input_size, self.num_layers, dropout)
...
--]]
function layer:sample(imgs, extra, opt)
  local sample_max = utils.getopt(opt, 'sample_max', 1)
  local beam_size = utils.getopt(opt, 'beam_size', 1)
  local temperature = utils.getopt(opt, 'temperature', 1.0)
  if sample_max == 1 and beam_size > 1 then return self:sample_beam(imgs, opt) end -- indirection for beam search --TODO: not changed
  ...
  for t=1,self.seq_length+2 do
    ...
    local inputs = {xt, extra, unpack(state)}
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+2 do
    ...
    if not can_skip then
      ...
      self.inputs[t] = {xt,extra,unpack(self.state[t-1])}
      -- forward the network
...
function layer:updateGradInput(input, gradOutput)
  ...
  local dextra -- grad table on extra input
  local dextra_table = {}
  ...
  for t=self.tmax,1,-1 do
    ...
    table.insert(dextra_table, dinputs[2])
  ...
  self.gradInput = {dimgs, dextra, torch.Tensor()}
