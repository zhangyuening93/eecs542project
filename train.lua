...
require 'loadcaffe'
-- local imports
local utils = require 'misc_new.utils'
require 'misc_new.DataLoader'
require 'misc_new.LanguageModel'
local net_utils = require 'misc_new.net_utils'
require 'misc_new.optim_updates'
require 'misc_new.SpatialCrossMapLRN'
...
cmd:option('-extra_input_size',512,'the encoding size of extra input')
...
cmd:option('-finetune_cnn_extra_after', -1, 'After what iteration do we start finetuning the CNN_EXTRA? (-1 = disable; never finetune, 0 = finetune from start)')
...
if string.len(opt.start_from) > 0 then
  ...
  net_utils.unsanitize_gradients(protos.cnn_extra)
  ...
  protos.cnn_extra = net_utils.build_cnn(cnn_raw, {encoding_size = opt.input_encoding_size, backend = cnn_backend})
...
local cnn_extra_params, cnn_extra_grad_params = protos.cnn_extra:getParameters()
print('total number of parameters in LM: ', params:nElement())
print('total number of parameters in CNN: ', cnn_params:nElement())
print('total number of parameters in CNN_EXTRA: ', cnn_extra_params:nElement())
assert(params:nElement() == grad_params:nElement())
assert(cnn_params:nElement() == cnn_grad_params:nElement())
print(cnn_grad_params:nElement())
print(cnn_extra_params:nElement())
print(cnn_extra_grad_params:nElement())
assert(cnn_extra_params:nElement() == cnn_extra_grad_params:nElement())
...
local thin_cnn_extra = protos.cnn_extra:clone('weight', 'bias')
-- sanitize all modules of gradient storage so that we dont save big checkpoints
net_utils.sanitize_gradients(thin_cnn)
net_utils.sanitize_gradients(thin_cnn_extra)
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local feats_extra = protos.cnn_extra:forward(data.images)
    local expanded_feats_extra = protos.expander:forward(feats_extra)
    local logprobs = protos.lm:forward{expanded_feats, data.labels, expanded_feats_extra}
    local loss = protos.crit:forward(logprobs, data.labels)
    loss_sum = loss_sum + loss
    loss_evals = loss_evals + 1

    -- forward the model to also get generated samples for each image
    local seq = protos.lm:sample(feats, feats_extra)
...
local function lossFun()
  protos.cnn:training()
  protos.cnn_extra:training()
  protos.lm:training()
  grad_params:zero()
  if opt.finetune_cnn_after >= 0 and iter >= opt.finetune_cnn_after then
    cnn_grad_params:zero()
  end
  if opt.finetune_cnn_extra_after >= 0 and iter >= opt.finetune_cnn_extra_after then
    cnn_extra_grad_params:zero()
  end
  ...
  local logprobs = protos.lm:forward{expanded_feats, data.labels, expanded_feats_extra} --TODO: change it to extra input
  ...
  local dexpanded_feats, dexpanded_feats_extra_table, ddummy = unpack(protos.lm:backward({expanded_feats, data.labels, expanded_feats}, dlogprobs))
  ...
  if opt.finetune_cnn_extra_after >= 0 and iter >= opt.finetune_cnn_extra_after then
    local dexpanded_feats_extra = nn.CAddTable()(dexpanded_feats_extra_table)
    local dfeats_extra = protos.expander:backward(feats_extra, dexpanded_feats_extra)
    local dx_extra = protos.cnn_extra:backward(data.images, dfeats_extra)
  end
...
while true do  
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    if best_score == nil or current_score > best_score then
      ...
      if iter > 0 then -- dont save on very first iteration
        ...
        save_protos.cnn_extra = thin_cnn_extra
  ...
  -- do 2 cnn update (if finetuning, and if rnn above us is not warming up right now)
  if opt.finetune_cnn_after >= 0 and iter >= opt.finetune_cnn_after then
  ...
  if opt.finetune_cnn_extra_after >= 0 and iter >= opt.finetune_cnn_extra_after then
    if opt.cnn_optim == 'sgd' then
      sgd(cnn_extra_params, cnn_extra_grad_params, cnn_learning_rate)
    elseif opt.cnn_optim == 'sgdm' then
      sgdm(cnn_extra_params, cnn_extra_grad_params, cnn_learning_rate, opt.cnn_optim_alpha, cnn_extra_optim_state)
    elseif opt.cnn_optim == 'adam' then
      adam(cnn_extra_params, cnn_extra_grad_params, cnn_learning_rate, opt.cnn_optim_alpha, opt.cnn_optim_beta, opt.optim_epsilon, cnn_extra_optim_state)
    else
      error('bad option for opt.cnn_optim')
    end
  end
